    ...
    fun onCreateOrderClicked() {
        if (_uiState.value?.paymentMethod != PaymentMethod.NOT_CHOSEN) {
            viewModelScope.launch {
                ...
                when (result) {
                    is com.game.internetshop.data.common.Result.Success -> {
                        _uiState.value = _uiState.value?.copy(isCreationSuccessful = true)
                        loadUserCartInfo(currentUserId)
                    }
                    is com.game.internetshop.data.common.Result.Error -> {
    ...
    fun onAddingToCart(productId: Int) {
        viewModelScope.launch {
            ...
            when (val result = addToCartUseCase(currentUserId, productId)) {
                is com.game.internetshop.data.common.Result.Success -> loadUserCartInfo(currentUserId)
                is com.game.internetshop.data.common.Result.Error -> {
                    _uiState.value = _uiState.value?.copy(errorMessage = result.message, isLoading = false)
                }
                is com.game.internetshop.data.common.Result.Loading -> _uiState.value = _uiState.value?.copy(isLoading = true)
            }
        }
    }
    ...
    private fun loadUserAndHisCart() {
        viewModelScope.launch {
            ...
            when(result) {
                ...
                is com.game.internetshop.data.common.Result.Error -> _uiState.value = _uiState.value?.copy(errorMessage = result.message, isLoading = false)
                is com.game.internetshop.data.common.Result.Loading -> _uiState.value = _uiState.value?.copy(isLoading = true)
            }
        }
    }
    ...
    private fun loadUserCartInfo(currentUserId: Int) {
        ...
        viewModelScope.launch {
            when (val result = getCartItemsUseCase(currentUserId)) {
                is com.game.internetshop.data.common.Result.Success -> {
                    ...
                    deferredProducts.forEach { deferred ->
                        when (val Result = deferred.await()) {
                            is com.game.internetshop.data.common.Result.Success -> {
                                selectedProductsFromDB.add(Result.data)
                            }
                            is com.game.internetshop.data.common.Result.Error -> {
                ...
                is com.game.internetshop.data.common.Result.Error -> _uiState.value = _uiState.value?.copy(errorMessage = result.message)
                is com.game.internetshop.data.common.Result.Loading -> _uiState.value = _uiState.value?.copy(isLoading = true)
            }
        }
